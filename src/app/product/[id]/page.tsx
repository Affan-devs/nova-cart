...
import { useCart } from "@/src/components/cartStore";
    ...
    if (!product) {
        return (
            <div className="min-h-screen flex items-center justify-center">
                <p className="font-dm text-[#999]">Product not found.</p>
            </div>
        );
    }
        ...
        <main className="bg-white min-h-screen">
            ...
            <div className="max-w-[1280px] mx-auto px-5 md:px-10 pt-28 pb-4">
                <nav className="flex items-center gap-2 font-dm text-[0.72rem] text-[#aaa] tracking-wide">
                    ...
                    <span>/</span>
                    <Link href="/shop" className="hover:text-[#555] transition-colors">Shop</Link>
                    <span>/</span>
            ...
            <div className="max-w-[1280px] mx-auto px-5 md:px-10 py-6 md:py-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start">
                    ...
                    <div className="md:pt-6 flex flex-col">
                        <p className="font-dm text-[0.7rem] tracking-[0.18em] uppercase text-[#aaa] mb-3">{product.category}</p>
                        <h1 className="font-playfair text-[2.2rem] md:text-[2.8rem] font-semibold text-[#1a1a1a] leading-tight">{product.title}</h1>
                        <p className="font-dm text-[0.82rem] text-[#888] tracking-wide mt-1">{product.subtitle}</p>

                        <p className="font-playfair text-[2rem] font-bold text-[#1a1a1a] mt-5">${product.price.toLocaleString()}</p>

                        <div className="h-px bg-[#f0ede8] my-6" />

                        <p className="font-dm text-[0.85rem] text-[#666] leading-relaxed">{product.description}</p>
                        ...
                        <button
                            onClick={handleAdd}
                            className="w-full h-[54px] bg-[#1a1a1a] text-white font-dm text-[0.82rem] font-semibold tracking-[0.14em] uppercase rounded-full hover:bg-[#3a3a3a] active:scale-[0.98] transition-all duration-200 flex items-center justify-center gap-3">
            ...
            <div className="max-w-[1280px] mx-auto px-5 md:px-10 py-16 md:py-24 border-t border-[#f0ede8] mt-8">
                <p className="font-dm text-[10px] text-center tracking-[1em] font-bold text-black uppercase mb-8">You May Also Like</p>
